...
curve.mlefunc<-function( par,y1,time.std)
{
  ...
  mu <- com.get_mu_co(curve.par, time.std)[1:length(time.std)]
...
double.H0 <- function(ny,times){
  ...
  sum.curve<-function(par10){
    A <- sum((cm-com.get_mu_co(par10,times)[1:length(times)])^2)
    A
  }
  init.par<-c(1, 30, -0.1, 1, 30, -0.1)
  ...
  while(loop_k<max_iter && max_err>epsi){
    ...
    r1 <- optim(c(parin[3:8]),mle.1,method = "BFGS",control=list(maxit=32000))    
    new1 <- r1$par
...
double.est <- function(dat,snp=dat$e.snp,y=dat$ep.p,interval=c(1,10)){
  ...
  for(i in n1:n2){
    ...
    H1 <- try(double.H1(par1,ny,nsnp,times),TRUE)
...
double.H1 <- function(par1,ny,nsnp,times){
  ...
  for(i in 1:length(snp.type)){
    ...
    sum.curve<-function(par10){
      A <- sum((SNP.p-com.get_mu_co(par10,mean.T)[1:length(mean.T)])^2)
      A
    }
    s.m <- optim(par1[3:8],sum.curve,method="BFGS",control=list(maxit=32000))
...
double.mlefunc <- function(par,y1,time.std,SNP.index=SNP.index,snp.type=snp.type)
{
  ...
  for(i in 1:length(snp.type)){
    ...
    yy1 <- y1[SNP.index[[i]],]
    mu <- com.get_mu_co(mu.g, time.std)[1:length(time.std)]
    nyy1 <- yy1
